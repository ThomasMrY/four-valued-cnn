...
def comput_threshold(input_d,steps):
	...
	threshhold_3_max = tf.constant(0.01)
	for j in range(steps):
		threshhold_2 = tf.constant(0.01)
		for i in range(steps):
			threshhold_3 = tf.constant(0.01)
			for i in range(steps):
				...
				output_temp_2 = tf.where(tf.greater(input_2,threshhold_2),0.5*tf.ones_like(input_2,tf.float32),tf.zeros_like(input_2,tf.float32))
				input_3 = tf.where(tf.greater(input_2,threshhold_2),tf.zeros_like(input_2,tf.float32),input_2)
				alpha_2_temp = tf.where(tf.greater(input_2,threshhold_2),0.5*tf.abs(input_2),tf.zeros_like(input_2,tf.float32))
				output_temp_3 = tf.where(tf.greater(input_3,threshhold_3),0.25*tf.ones_like(input_3,tf.float32),tf.zeros_like(input_3,tf.float32))
				alpha_3_temp = tf.where(tf.greater(input_3,threshhold_2),0.25*tf.abs(input_3),tf.zeros_like(input_3,tf.float32))
				output_temp = tf.add(tf.add(output_temp_1,0.5*output_temp_2),0.25*output_temp_3)
				alpha_temp = (tf.reduce_sum(alpha_1_temp)+tf.reduce_sum(alpha_2_temp)+tf.reduce_sum(alpha_3_temp))
				...
				threshhold_3_max = tf.where(tf.greater(f_max_new,f_max),threshhold_3,threshhold_3_max)
				f_max = tf.where(tf.greater(f_max_new,f_max),f_max_new,f_max)
				threshhold_3 += threshhold_2/steps
			threshhold_2 += threshhold_1/steps
		threshhold_1 += 2/steps
	return threshhold_1_max,threshhold_2_max,threshhold_3_max

def add_fourvalue(input_d,steps):
	threshhold_1,threshhold_2,threshhold_3 = comput_threshold(input_d,steps)
	...
	output_temp_3 = tf.where(tf.greater(input_3,threshhold_3),0.25*tf.ones_like(input_3,tf.float32),tf.zeros_like(input_3,tf.float32))
	alpha_3_temp = tf.where(tf.greater(input_3,threshhold_3),0.25*tf.abs(input_3),tf.zeros_like(input_3,tf.float32))
	
	alpha_temp = (tf.reduce_sum(alpha_1_temp)+tf.reduce_sum(alpha_2_temp)+tf.reduce_sum(alpha_3_temp))
	output_temp = tf.add(tf.add(output_temp_1,0.5*output_temp_2),0.25*output_temp_3)
	
	output = tf.add(tf.add(output_temp_1,output_temp_2),output_temp_3)
...
layer3_t,alpha_1 = add_fourvalue(layer3,20)
...
layer4_t,alpha_2 = add_fourvalue(layer4_drop,20)
